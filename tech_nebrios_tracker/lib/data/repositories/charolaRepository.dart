...
class CharolaRepository {
  ...
  /// Obtiene una lista paginada de charolas desde la API.
  /// Retorna un [Map] con la respuesta decodificada, o lanza excepciones en caso de error.
  Future<Map<String, dynamic>?> obtenerCharolasPaginadas(
    int pag,
    int limite,
  ) async {
    final token = await _userUseCases.obtenerTokenActual(); // Obtener token actual del usuario
    final uri = Uri.parse(
  ...
  /// Convierte la respuesta cruda de la API a un modelo [CharolaDashboard].
  /// Si la respuesta es válida, retorna el objeto; si no, retorna null.
